...
public class Tornado : Cell
{
    ...
    public void Update()
    {
        bool should_delete = GameCore.Random.Next(0, CellStats.TORNADO_DELETION_FACTOR) == 0;
        lifetime--;
        if (lifetime <= 0 && should_delete == true)
        {
            game_world.RemoveCell(this);
        }

        bool left_bias = GameCore.Random.Next(0, 2) == 0;
        bool should_swap = GameCore.Random.Next(0, CellStats.TORNADO_FLOW_FACTOR) == 0;
